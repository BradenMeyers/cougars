...
if [ "$(uname -m)" == "aarch64" ]; then
    ...
    tmux send-keys -t $SESSION_NAME:0.1 "bash launch.sh bluerov" C-m

    tmux new-window -t $SESSION_NAME:1 -c "/home/frostlab/oculus/blueprint_oculus_sonar_driver" -n "oculus"
    tmux send-keys -t $SESSION_NAME:1 "./build/bin/run -h" C-m
    tmux send-key -t $SESSION_NAME:1 "./build/bin/run " 
fi
